...
kssl_error_code parse_header(BYTE *bytes /*Stream of bytes containing a kssl_header*/ ,
	kssl_header *header/* Returns the populated header (must be allocated by caller)*/) 
{  
	int offset = 0;
...
kssl_error_code flatten_operation(kssl_header *header, kssl_operation *operation, BYTE **out_operation, int *length) 
{             
  int local_req_len;
  BYTE *local_req;
  int offset = 0;
  int padding_size = 0;
  if (header == NULL || operation == NULL || out_operation == NULL || length == NULL)
  {
		return KSSL_ERROR_INTERNAL;
  }
  ...
  // The operation will always be padded to KSSL_PAD_TO + KSSL_ITEM_HEADER_SIZE bytes
  ...
  if (operation->is_payload_set) 
  {
    flatten_item(KSSL_TAG_PAYLOAD, operation->payload, operation->payload_len, local_req, &offset);
  }
  if (operation->is_ski_set)
  {
    	flatten_item(KSSL_TAG_SKI, operation->ski, KSSL_SKI_SIZE, local_req, &offset);
  }
  if (operation->is_digest_set)
  {
    flatten_item(KSSL_TAG_DIGEST, operation->digest, KSSL_DIGEST_SIZE, local_req, &offset);
  }
  if (operation->is_ip_set)
  {
    flatten_item(KSSL_TAG_CLIENT_IP, operation->ip, operation->ip_len, local_req, &offset);
  }
...
kssl_error_code parse_message_payload(BYTE *payload, int len, kssl_operation *operation) 
{ 
	int offset = 0;
	kssl_item temp_item;
	...
	while (offset < len) 
	{
		...
		if (parse_item(payload, &offset, &temp_item) != KSSL_ERROR_NONE || len < offset)
		{
			return KSSL_ERROR_FORMAT;
		}
